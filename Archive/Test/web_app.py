...
# Attrition & Retention # 
# ----------------------------- # 
with tabs[1]: st.subheader("Attrition & Retention Overview") 
retention_year = df_raw[df_raw["Calendar Year"] == selected_year] 
if not retention_year.empty: 
    ...
    fig = px.line(attrition_summary, x="Calendar Year", y="ResignedFlag", markers=True, title="Resignations per Year") 
    ...
    fig = px.bar(gen_retention, x="Generation", y="Retention", title="Retention Rate by Generation") 
    st.plotly_chart(fig, width="stretch", height=300)
...
with tabs[3]:
    ...
    heat_year = heat[heat["Year"] == selected_year]
    st.dataframe(heat_year)
    fig = px.imshow(heat.set_index("Year").T, aspect="auto", color_continuous_scale="RdYlGn", title="Satisfaction Heatmap")
    st.plotly_chart(fig, width="stretch")
    st.subheader("Engagement Index")
    ei = df["Engagement Index"]
    ei_year = ei[ei["Year"] == selected_year]
    st.dataframe(ei_year)
    fig = px.line(ei, x="Year", y="Engagement Score", markers=True, title="Engagement Index Trend")
    st.plotly_chart(fig, width="stretch")
    st.subheader("Driver Analysis (Resignation)")
    driver_res = df["Driver-Resignation"]
    driver_res_year = driver_res[driver_res["Year"] == selected_year]
    st.dataframe(driver_res_year)
    fig = px.bar(driver_res_year, x="Category", y="Correlation with Resignation", color="Year", title=f"Driver Analysis - Resignation ({selected_year})")
    st.plotly_chart(fig, width="stretch")
    st.subheader("Driver Analysis (Promotion)")
    driver_prom = df["Driver-Promotion"]
    driver_prom_year = driver_prom[driver_prom["Year"] == selected_year]
    st.dataframe(driver_prom_year)
    fig = px.bar(driver_prom_year, x="Category", y="Correlation with Promotion", color="Year", title=f"Driver Analysis - Promotion ({selected_year})")
    st.plotly_chart(fig, width="stretch")
...
with tabs[4]:
    ...
    # risk_year = risk[risk["Year"] == selected_year]
    # st.dataframe(risk_year)
    st.subheader("Engagement vs Retention")
    evr = df["Engagement vs Retention"]
    evr_year = evr[evr["Year"] == selected_year]
    st.dataframe(evr_year)
    fig = px.bar(evr_year, x="Resignee Checking ", y="Avg Engagement Score", color="Resignee Checking ", title=f"Engagement vs Retention ({selected_year})")
    st.plotly_chart(fig, width="stretch")