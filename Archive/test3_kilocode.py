...
# Gender percentage data by year
gender_data_by_year = {
    2019: [62, 38],
    2020: [62, 38],
    2021: [60, 40],
...
with col1:
    hc_display = f"{total_hc:,}" if total_hc > 0 else "--"
    st.markdown(f"""
    <div class="metric-box">
        <div class="metric-box-value">{hc_display}</div>
        <div class="metric-box-label">Total Headcount</div>
    </div>
    """, unsafe_allow_html=True)

with col2:
    mu_display = f"{managers_up:,}" if managers_up > 0 else "--"
    st.markdown(f"""
    <div class="metric-box">
        <div class="metric-box-value">{mu_display}</div>
        <div class="metric-box-label">Managers & Up</div>
    </div>
    """, unsafe_allow_html=True)

with col3:
    as_display = f"{associates:,}" if associates > 0 else "--"
    st.markdown(f"""
    <div class="metric-box">
        <div class="metric-box-value">{as_display}</div>
...
with col4:
    st.markdown(f"""
    <div class="metric-box">
        <div class="metric-box-value">{tenure_display}</div>
...
with col5:
    st.markdown(f"""
    <div class="metric-box">
        <div class="metric-box-value">{retention_display}</div>
...
with col1:
    ...
    # Handle selection to update display_year and selected_level
    if selected and 'selection' in selected and selected['selection']['points']:
        points = selected['selection']['points']
        if points:
            clicked_year = points[0]['x']
            clicked_level = points[0]['legendgroup']
            st.session_state.last_clicked_year = clicked_year
            st.session_state.selected_level = clicked_level
            st.rerun()
...
with col1:
    ...
    promo_data = pd.DataFrame({
        'Year': years,
        'Opportunities': [450, 500, 700, 900, 1000, 1400, 1700]
    })
    fig3 = px.bar(
        promo_data,
        x='Year',
...
with col1:
    ...
    if display_year in gender_data_by_year:
        percentages = gender_data_by_year[display_year]
    else:
        percentages = [59, 41]  # default to 2025
...
with col2:
    ...
    if display_year in gen_count_by_year:
        counts = gen_count_by_year[display_year]
    else:
        counts = [0, 221, 759, 420]  # default to 2025
